<template>
  <div class="computational-design">
...
export default {
  ...
  setup() {
    ...
    const computationalDesignProjects = ref([])
    ...
    const loadComputationalDesignProjects = async () => {
      const projectEntries = Object.entries(projects)
        .filter(([key, project]) => project.category === 'computational-design')
        ...
        projectEntries.map(async ([key, project]) => {
          try {
            ...
            const [galleryFiles, thumbnailFile, appEmbeddingFile] = await Promise.all([
              ...
              getThumbnailFilenameAuto(key, 'computational-design'),
      ...
      computationalDesignProjects.value = projectsWithImages
    }

    // Selected project data
    const selectedProjectData = computed(() => {
      return computationalDesignProjects.value.find(p => p.id === selectedProject.value)
    })
    ...
    const loadSelectedProjectMetadata = async (projectId) => {
      ...
      try {
        ...
        if (baseProject) {
          const metadata = await getProjectMetadata(projectId, 'computational-design', baseProject)
    ...
    const handleProjectSelection = async (projectId) => {
      // Only update if it's a computational-design project
      const project = projects[projectId]
      if (project && project.category === 'computational-design') {
    ...
    onMounted(async () => {
      ...
      // Load computational-design projects with auto-detection
      await loadComputationalDesignProjects()
